...
const Terms_condition = () => {
    ...
    api
      ...
      .then((result) => {
        if (result.data.result.length === 0) {
          ...
          setAboutcontent(result.data.result[0].termscontent);
        }
        setLoading(false);
      })
      .catch((err) => {
  ...
  const handleeditdata = (e) => {
    ...
    formData.append("termscontent", aboutcontent);
    if (adddata) {
      if (!aboutcontent) {
      ...
      if (!aboutcontent) {
        ...
        setLoading(true);
        api
          .put(`terms/terms_update/${id}`, formData)
          .then((result) => {
